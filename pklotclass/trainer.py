...
class Trainer:
    def __init__(self, model, criterion, optimizer, train_dataloader, valid_dataloader, test_dataloader, scheduler, cfg):
    ...
    def train(self):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            self.logger.info('-' * 110)
            self.logger.info('| End of epoch {:3d} | time: {:5.2f}s | '.format(epoch, time.time() - epoch_start_time))
            self.logger.info(f'| {"Accuracy":>10} | {"TPR":>10} | {"TNR":>10} | {"PPV":>10} | {"NPV":>10} |')
            self.logger.info('| {:10.3f} | {:10.3f} | {:10.3f} | {:10.3f} | {:10.3f} |'.format(accu_val, TPR, TNR, PPV, NPV))
            self.logger.info('-' * 110 + '\n')
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            self.logger.info(f'| {"Accuracy":>10} | {"TPR":>10} | {"TNR":>10} | {"PPV":>10} | {"NPV":>10} |')
            self.logger.info('| {:10.3f} | {:10.3f} | {:10.3f} | {:10.3f} | {:10.3f} |'.format(total_acc / total_count, TPR, TNR, PPV, NPV))
            self.logger.info('-' * 110)
    ...
    def _resume_checkpoint(self, resume_path):
        ...
        if checkpoint['arch'] != self.cfg_global['model']['type']:
            self.logger.warning('Warning: Architecture configuration from the config file and the checkpoint is different. This may yield an exception while state_dict is loaded.')
